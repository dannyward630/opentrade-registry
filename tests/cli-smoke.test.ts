...
describe("opentrade CLI", () => {
  ...
  it("lists, shows, and validates sources", () => {
    ...
    expect(runCli(["sources", "validate"]).stdout).toContain("Validated 64 source registry entries.");
  ...
  it("filters source listings for discovery workflows", () => {
    ...
    expect(blockedJson).toHaveLength(55);
  ...
  it("summarizes terminal source readiness", () => {
    ...
    expect(readiness).toContain("sources: 64");
    expect(readiness).toContain("implemented adapter sources: 9");
    expect(readiness).toContain("terminal source decisions: 64");
    expect(readiness).toContain("blocked sources: 55");
    ...
    expect(readiness).toContain("- blocked: 55");
    ...
    expect(json.sourceCount).toBe(64);
    ...
    expect(json.sourcesByResearchOutcome.blocked).toBe(55);
