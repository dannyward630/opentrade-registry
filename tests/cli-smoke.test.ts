...
describe("opentrade CLI", () => {
  ...
  it("lists, shows, and validates sources", () => {
    ...
    expect(runCli(["sources", "validate"]).stdout).toContain("Validated 62 source registry entries.");
  ...
  it("filters source listings for discovery workflows", () => {
    ...
    expect(blockedJson).toHaveLength(53);
  ...
  it("summarizes terminal source readiness", () => {
    ...
    expect(readiness).toContain("sources: 62");
    expect(readiness).toContain("implemented adapter sources: 9");
    expect(readiness).toContain("terminal source decisions: 62");
    expect(readiness).toContain("blocked sources: 53");
    ...
    expect(readiness).toContain("- blocked: 53");
    ...
    expect(json.sourceCount).toBe(62);
    ...
    expect(json.sourcesByResearchOutcome.blocked).toBe(53);
