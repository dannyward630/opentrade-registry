...
describe("opentrade CLI", () => {
  ...
  it("lists, shows, and validates sources", () => {
    ...
    expect(list).toContain("us.as.dpw.contractors");
    ...
    expect(list).toContain("us.mp.dpw.building_safety_code");
    ...
    expect(list).toContain("us.or.bcd.trade_contractors");
    expect(list).toContain("us.or.ccb.active_licenses");
    expect(list).toContain("us.or.deq.asbestos_contractors");
    ...
    expect(list).toContain("us.id.deq.asbestos_compliance");
    expect(list).toContain("us.id.dopl.contractors");
    expect(list).toContain("us.id.dopl.electrical");
    expect(list).toContain("us.id.dopl.hvac");
    expect(list).toContain("us.id.dopl.plumbing");
    expect(list).toContain("us.id.dopl.public_works_contractors");
    expect(list).toContain("us.ia.dial.asbestos_licenses");
    expect(list).toContain("us.ia.dial.electrical_licensing");
    expect(list).toContain("us.ia.dial.plumbing_mechanical");
    ...
    expect(list).toContain("us.ny.dol.asbestos_contractors");
    ...
    expect(westVirginia).toContain("West Virginia Division of Labor Contractor and Trade License Search");
    ...
    const northernMarianaIslands = runCli(["sources", "show", "us.mp.dpw.building_safety_code"]).stdout;
    expect(northernMarianaIslands).toContain("CNMI Department of Public Works Building Safety Code Division");
    expect(northernMarianaIslands).toContain("manual_public_records_file");
    expect(northernMarianaIslands).toContain("maturity: blocked");
    const oregonBcd = runCli(["sources", "show", "us.or.bcd.trade_contractors"]).stdout;
    expect(oregonBcd).toContain("Oregon BCD Trade Contractor and Business Licenses");
    expect(oregonBcd).toContain("active-license tab-delimited ASCII file, updated daily");
    const oregonDeq = runCli(["sources", "show", "us.or.deq.asbestos_contractors"]).stdout;
    expect(oregonDeq).toContain("Oregon DEQ Licensed Asbestos Abatement Contractors");
    expect(oregonDeq).toContain("maturity: blocked");
    const iowaPmsb = runCli(["sources", "show", "us.ia.dial.plumbing_mechanical"]).stdout;
    expect(iowaPmsb).toContain("Iowa Plumbing and Mechanical Systems Board Licensing");
    expect(iowaPmsb).toContain("maturity: blocked");
    expect(iowaPmsb).toContain("plumbing");
    expect(iowaPmsb).toContain("sheet-metal");
    const iowaAsbestos = runCli(["sources", "show", "us.ia.dial.asbestos_licenses"]).stdout;
    expect(iowaAsbestos).toContain("Iowa DIAL Asbestos Licenses and Permits");
    expect(iowaAsbestos).toContain("bulk_json");
    expect(iowaAsbestos).toContain("weekly");
    const idahoElectrical = runCli(["sources", "show", "us.id.dopl.electrical"]).stdout;
    expect(idahoElectrical).toContain("Idaho DOPL Electrical Board License Search");
    expect(idahoElectrical).toContain("maturity: blocked");
    const idahoAsbestos = runCli(["sources", "show", "us.id.deq.asbestos_compliance"]).stdout;
    expect(idahoAsbestos).toContain("Idaho DEQ Asbestos Renovation and Demolition Compliance");
    expect(idahoAsbestos).toContain("manual_public_records_file");
    const indianaPlumbing = runCli(["sources", "show", "us.in.pla.plumbing"]).stdout;
    expect(indianaPlumbing).toContain("Indiana Plumbing Commission License Verification");
    expect(indianaPlumbing).toContain("maturity: blocked");
    ...
    const oklahomaAsbestos = runCli(["sources", "show", "us.ok.labor.asbestos_contractors"]).stdout;
    expect(oklahomaAsbestos).toContain("Oklahoma Department of Labor Asbestos Abatement Licensing");
    expect(oklahomaAsbestos).toContain("no stable public roster");
    ...
    expect(runCli(["sources", "validate"]).stdout).toContain("Validated 163 source registry entries.");
  ...
  it("filters source listings for discovery workflows", () => {
    ...
    expect(blockedJson).toHaveLength(154);
  ...
  it("summarizes terminal source readiness", () => {
    ...
    expect(readiness).toContain("sources: 163");
    expect(readiness).toContain("implemented adapter sources: 9");
    expect(readiness).toContain("terminal source decisions: 163");
    expect(readiness).toContain("blocked sources: 154");
    ...
    expect(readiness).toContain("- blocked: 154");
    ...
    expect(json.sourceCount).toBe(163);
    ...
    expect(json.sourcesByResearchOutcome.blocked).toBe(154);
  ...
  it("summarizes state and territory source coverage", () => {
    ...
    expect(coverage).toContain("- blocked: 48");
    expect(coverage).toContain("- local_file_supported: 1");
    expect(coverage).toContain("- network_opt_in_supported: 2");
    ...
    expect(json.states.find((row: { state: string }) => row.state === "MN").sourceIds).toEqual([
      "us.mn.dli.licenses_registrations",
      "us.mn.mdh.asbestos_contractors",
    ]);
    expect(json.states.find((row: { state: string }) => row.state === "OR").sourceIds).toEqual([
