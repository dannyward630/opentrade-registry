...
describe("hosted API", () => {
  ...
  it("filters source registry entries from local metadata", async () => {
    ...
    expect(blocked.body.count).toBe(91);
  ...
  it("returns source readiness from local metadata", async () => {
    ...
    expect(response.body).toMatchObject({
      ...
      sourceCount: 100,
      terminalSourceCount: 100,
      blockedSourceCount: 91,
    ...
    expect(response.body.sourcesByResearchOutcome.blocked).toBe(91);
  });
