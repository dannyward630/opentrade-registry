...
describe("web status build", () => {
  it("emits source and territory coverage snapshots from registry files", async () => {
    ...
    expect(result.stdout).toContain("Built OpenTrade web status page with 89 sources.");
    ...
    expect(readiness.sourceCount).toBe(89);
    expect(readiness.registryOnlySourceCount).toBe(0);
    expect(readiness.terminalSourceCount).toBe(89);
    expect(readiness.blockedSourceCount).toBe(80);
