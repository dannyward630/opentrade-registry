...
describe("web status build", () => {
  it("emits source and territory coverage snapshots from registry files", async () => {
    ...
    expect(result.stdout).toContain("Built OpenTrade web status page with 94 sources.");
    ...
    expect(readiness.sourceCount).toBe(94);
    expect(readiness.registryOnlySourceCount).toBe(0);
    expect(readiness.terminalSourceCount).toBe(94);
    expect(readiness.blockedSourceCount).toBe(85);
