...
describe("web status build", () => {
  it("emits source and territory coverage snapshots from registry files", async () => {
    ...
    expect(result.stdout).toContain("Built OpenTrade web status page with 142 sources.");
    ...
    expect(sources).toHaveLength(142);
    expect(sources.map((source) => source.id)).toContain("us.pr.daco.contractors");
    expect(readiness.sourceCount).toBe(142);
    expect(readiness.registryOnlySourceCount).toBe(0);
    expect(readiness.terminalSourceCount).toBe(142);
    expect(readiness.blockedSourceCount).toBe(133);
