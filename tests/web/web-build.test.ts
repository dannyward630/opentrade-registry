...
describe("web status build", () => {
  it("emits source and territory coverage snapshots from registry files", async () => {
    ...
    expect(result.stdout).toContain("Built OpenTrade web status page with 128 sources.");
    ...
    expect(readiness.sourceCount).toBe(128);
    expect(readiness.registryOnlySourceCount).toBe(0);
    expect(readiness.terminalSourceCount).toBe(128);
    expect(readiness.blockedSourceCount).toBe(119);
