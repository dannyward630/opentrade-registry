...
describe("web status build", () => {
  it("emits source and territory coverage snapshots from registry files", async () => {
    ...
    expect(result.stdout).toContain("Built OpenTrade web status page with 61 sources.");
    ...
    expect(readiness.sourceCount).toBe(61);
    expect(readiness.registryOnlySourceCount).toBe(0);
    expect(readiness.terminalSourceCount).toBe(61);
    expect(readiness.blockedSourceCount).toBe(52);
