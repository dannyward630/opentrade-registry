...
describe("web status build", () => {
  it("emits source and territory coverage snapshots from registry files", async () => {
    ...
    expect(result.stdout).toContain("Built OpenTrade web status page with 129 sources.");
    ...
    expect(readiness.sourceCount).toBe(129);
    expect(readiness.registryOnlySourceCount).toBe(0);
    expect(readiness.terminalSourceCount).toBe(129);
    expect(readiness.blockedSourceCount).toBe(120);
