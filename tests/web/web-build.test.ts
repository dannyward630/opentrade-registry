...
describe("web status build", () => {
  it("emits source and territory coverage snapshots from registry files", async () => {
    ...
    expect(result.stdout).toContain("Built OpenTrade web status page with 127 sources.");
    ...
    expect(readiness.sourceCount).toBe(127);
    expect(readiness.registryOnlySourceCount).toBe(0);
    expect(readiness.terminalSourceCount).toBe(127);
    expect(readiness.blockedSourceCount).toBe(118);
