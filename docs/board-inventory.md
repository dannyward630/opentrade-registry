...
The inventory currently has `completeness: "representative_source_baseline"`. Its 146 rows migrate every existing state, DC, and major-territory source into the board model. This proves source linkage and terminal access-path handling, but it does not yet claim every material statewide trade board has been identified.

The board-coverage ledger currently resolves `588` of `784` jurisdiction/domain decisions, leaving `196` for research. New Hampshire is fully resolved with OPLC electrician and mechanical-safety boards, DES asbestos and water-well evidence, and local-only building-code boundaries. Municipal licensing remains excluded.
