...
The inventory currently has `completeness: "representative_source_baseline"`. Its 138 rows migrate every existing state, DC, and major-territory source into the board model. This proves source linkage and terminal access-path handling, but it does not yet claim every material statewide trade board has been identified.

The board-coverage ledger currently resolves `560` of `784` jurisdiction/domain decisions, leaving `224` for research. West Virginia is fully resolved after Virginia with Labor contractor, HVAC, plumber, and OEHS asbestos paths. Municipal licensing remains excluded.
