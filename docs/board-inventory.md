...
The inventory currently has `completeness: "representative_source_baseline"`. Its 72 rows migrate every existing state, DC, and major-territory source into the board model. This proves source linkage and terminal access-path handling, but it does not yet claim every material statewide trade board has been identified.
